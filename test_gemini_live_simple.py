    ...
    for model_name in models_to_try:
        try:
            ...
            session_context = client.aio.live.connect(
                model=model_name,
                # No config to avoid payload issues
            )

            print("⏳ Waiting for session to establish...")
            session = await asyncio.wait_for(session_context.__aenter__(), timeout=30.0)
            ...
            print(f"❌ Failed to connect with {model_name}: {e}")
            print(f"❌ Error type: {type(e).__name__}")
            continue

    print("❌ All models failed to connect")
    return False
